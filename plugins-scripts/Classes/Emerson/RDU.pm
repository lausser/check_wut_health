...
sub init {
  ...
  if ($self->mode =~ /device::sensor::status/) {
    $self->analyze_and_check_sensor_subsystem("Classes::Emerson::RDU::Component::SensorSubsystem");
  } elsif ($self->mode =~ /device::hardware::health/) {
    $self->analyze_and_check_diag_subsystem("Classes::Emerson::RDU::Component::EnvironmentalSubsystem");
  } else {
    $self->no_such_mode();
  }
}
