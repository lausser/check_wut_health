...
sub init {
  ...
  $self->get_snmp_tables("STULZ-WIB8000-MIB", [
      ["units", "unitstateTable", "Classes::Stulz::GenericUnit", undef, ["hardwareTypeControllerType"]],
  ]);
...
package Classes::Stulz::GenericUnit;
...
sub finish {
  ...
  if ($self->{hardwareTypeControllerType} eq "C1002") {
    $self->rebless("Classes::Stulz::C1002");
#  } elsif ($self->{hardwareTypeControllerType} eq "C7000IOC") {
#    $self->rebless("Classes::Stulz::C7000IOC");
#    diese monstrositaet kommt spaeter
  } else {
    $self->rebless("Classes::Stulz::Unit");
  }
  $self->finish();
}
